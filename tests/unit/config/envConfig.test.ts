...
describe('Config', () => {
  ...
  describe('URL Normalization', () => {
    it('should normalize urls when valid urls provided', async () => {
      // Arrange
      process.env.CORS_ALLOWED_ORIGINS = 'HTTPS://Example.COM/path/,HTTP://LOCALHOST:3000/';
      const { Config } = await import('@config');
    ...
    it('should remove trailing slashes when urls have trailing slashes', async () => {
      // Arrange
      process.env.CORS_ALLOWED_ORIGINS = 'https://example.com/,https://app.example.com/api/';
      const { Config } = await import('@config');
    ...
    it('should preserve invalid urls when invalid urls provided', async () => {
      ...
      process.env.CORS_ALLOWED_ORIGINS = 'https://valid.com,invalid url,mailto:test@example.com';
      const { Config } = await import('@config');
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle missing package version when package json invalid', async () => {
      ...
      // Assert
      expect(config.version).toBe('1.0.0 test'); // From our mock, or '0.0.0' if truly missing
    });
  });
